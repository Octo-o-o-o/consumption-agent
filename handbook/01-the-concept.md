...
**Calibration exercises.** TikTok's For You feed = a consumption agent, *owned by the platform* (satisfies trait 2, fails 3 and 4). The spam filter = the first mass-deployed consumption agent in history (all four traits, minimal intelligence). Ad blockers and SponsorBlock = the prehistoric form. NotebookLM = close, but transformation only. ChatGPT in its default use = production. **Hybrids are legitimate** (read the material, then draft an action list): the boundary is drawn by intent and destination, and it is a spectrum, not a wall.
